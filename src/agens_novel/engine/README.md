...
- `turn_runner.py` — 项目内 Agent 图调用器，每回合执行 `run_turn_sync`。
- `render.py` — UI 无关的文本格式化；Web 当前只使用状态条和日志类输出，旧工具面板查询不再作为产品入口。
- `_stream_context.py` — Thread-local 流式回调上下文，**不要**写入 Agent state。
...
| `run_turn_sync(agent, input, session)` | `-> dict` | 调用 LLM Agent 图入口 |
...
- **回调不写入 state**：流式回调通过 `_stream_context` 传递，不要塞进 Agent state。
- **Agent state 仅保存数据**：使用 dict / list / int / str / bool / float；不可存 callable、file handle、threading.Lock 等。
