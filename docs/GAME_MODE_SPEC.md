...
> 状态：**v5 / 核心已落地，端到端收口中**。游戏模式核心规则已切换到 A/B/C/D 四按钮、无 HP/MP、事件判定战斗、寿元寿命表、六维属性和动态流逝年数；服务层、迁移和 React 入口仍需按本表持续验收。
...
| 8 | 游客与账号存档验收 | 游客可玩无云存档，账号可存读档 | ⏳ 本地 API 覆盖中，仍需真实浏览器联调 |
