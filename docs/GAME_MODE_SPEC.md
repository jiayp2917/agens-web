...
- **标准局长**：新局默认使用 `story_version=2` 的 90 回合内容，允许死亡、寿尽、失败或飞升提前结束；黄金路线必须能在第 90 回合前飞升。`story_version=1` 的 60 回合内容仅用于旧存档兼容。`story_version=3` 同样在第 90 回合收束主线成功或失败，但进入 `post_arc` 而不自动 `game_over`；真正终局仍是飞升、死亡、寿终或玩家主动结束。
...
- **版本化局长**：每个内容版本声明自身收束条件和目标回合范围；完整局验收以规则终局为准。v3 的第 91 回合起只使用独立 `post_arc` 事件池，不重复第九阶段、承诺或结局文本。旧存档继续按精确版本解析，不静默切换版本。
...
- **长期主线**：四套世界分别绑定版本化主线。v2 按九阶段推进，并在自身声明的终局条件形成路线相关结局；v3 在每局生成恰好两条命数承诺，并按早期钩子、中期压力、后期兑现或失败推进。20 回合切片只检查入局与扎根阶段的目标和持续后果，不得提前标记主线完成。
- **剧情存档**：存档保存 `story_key`、`story_version` 和可变 `story_state`（阶段、目标、路线计数、压力、未决线索、势力态度、承诺和近期 beat），不复制不可变剧情定义，也不静默切换版本。
- **Live 契约验收**：Narrator 正文和四个选项必须同时完整；正文仍含无法安全转写的英文时，该回合不得计作 live-model 成功。兼容 `state_update` 不属于权威或 strict 字段，只能作诊断。HTTP 200、回合推进或本地 fallback 都不能替代该门禁。
...
- 模型输出中的 `state_update` 只可作为兼容诊断；规则引擎不会把它合并进权威状态。模型若声称获得、升层、受伤或结缘，必须与本回合 `RuleTurnOutcomeV1` 的结构化结果一致，否则叙事被拒绝或改写。
- 生成结局墓志铭或飞升总结。
...
Narrator 每次必须提供两个 strict 语义段：

1. 非空的第三人称编年史正文。
2. 恰好四个非空且互不重复的中文行动选项，依次对应稳妥、机遇、风险、气运。

内部对象为 `NarratorEnvelopeV1(narrative, choices)`。传输格式不是玩法规范：支持 provider JSON schema 时可使用 JSON envelope；不支持时使用 JSON object 或兼容标签格式。应用层必须把各种传输统一解析成相同内部对象。旧 `state_update` 可以读取用于诊断，但不得绕过 `ChoiceIntentV1 -> RuleTurnOutcomeV1` 的规则结算。缺少任一 strict 语义段、含结构化残留、英文状态词或无效选项均属于契约失败。
