...
1. 为模型 URL 校验增加可插拔的固定解析/连接层，作为现有应用校验、Squid 和主机 ACL 之外的额外防线。
2. 扩展前端测试到完整账号流程、长文本和恢复后继续游玩；HTTP 409 单元测试与真实键盘/焦点证据已完成。
3. `requirements-dev.txt` 和过时路线文档已删除；`Makefile` 与 `scripts/import_web_data_to_pg.py` 暂时保留，等待负责人确认仓库外 Linux 入口和旧数据迁移流程均不再依赖它们。未获得确认前不得删除。

2026-07-23 已完成并移出 backlog：灵根注册表与寿元显示一致性、评估钩子/应用工厂解耦、两个运行时依赖环、浏览器验收驱动与裁决拆分、版本化剧情/开局/模型/会话/Web 服务边界拆分，以及 Web API、会话和失败路径测试拆分。每项均保持 v1/v2/v3 存档、固定种子、单次规则结算、重试、事件顺序、幂等和 HTTP 409 回归覆盖。
