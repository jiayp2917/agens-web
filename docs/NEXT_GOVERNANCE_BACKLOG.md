...
1. **恢复经批准的本地 PostgreSQL 测试端点**
   - 必需的 `127.0.0.1:55432` 没有监听器。已发现的 PostgreSQL 服务已停止，配置为 `5432` 且监听地址过宽，因此本批没有启动或改写它。
   - 在宣称本地数据库或浏览器验证通过前，须识别或配置一套明确只监听 loopback `55432` 的 PostgreSQL 实例，将仅测试用 URL 注入测试进程后串行运行数据库套件。不得以 `5432` 替代。

2. **当前普通运行链上的真实模型资格验收仍未完成**
   - 旧的独立评估结果只属于当时的提交和已删除的运行面，不能作为当前代码的资格结论。
   - 后续 Agens、DeepSeek 连通性、v2 smoke 和 v3 场景都必须启动普通 `web.backend.app`，使用当前 PostgreSQL 系统模型配置和同一内部 envelope/strict 统计。
   - 真实调用必须单独标为 `llm_real`；fallback、repair 或 contract recovery 不能计为 strict 成功。未完成当前链路的完整局和内容审阅前，不得报告模型优劣、能力百分比或内容质量排名。
