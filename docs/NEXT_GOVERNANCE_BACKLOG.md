...
1. **恢复唯一的本机 PostgreSQL 连接串并重跑数据库门禁**
   - 本地开发、测试和浏览器验证共用 `DATABASE_URL` 指向的唯一数据库；数据库测试允许清库或重建 `public` schema。
   - 在宣称本地数据库或浏览器验证通过前，须为当前 PostgreSQL 服务提供可用的本机 `DATABASE_URL`，并串行运行数据库、备份恢复和浏览器套件。
