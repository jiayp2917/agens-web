...
- 最新本地代码基线：已包含脱敏模型性能观测、narrator prompt history soft-cap、2026-07-04 属性尺度清理、动态开局生成链路、2026-07-06 治理审计执行，以及 2026-07-07 防御性运行修复（malformed nested `state_delta` 不崩溃、`fallback_prompt.active` 不再被历史 `model_failure` 污染且 runner 重建可恢复当前提示态、`/choice` 接受 A/B/C/D 与 `"1"`-`"4"`）；`compileall` passed，`tests\web` 73 passed，全量 `pytest -q` 508 passed，前端 build passed，`git diff --check` 仅 LF/CRLF warning。
- 最新本地真实 Chrome 验收：2026-07-07 `local-visible-history-softcap-c1d8628-20260707` 通过（动态开局 live start gate、普通账号注册/登录、角色创建、20/20 choice non-fallback、存档/读档均通过；repair 0/20、judge 5 次、fallback 0；平均回合约 39.3s、最大约 157.3s）。证据位于 `output/playwright/local-visible-history-softcap-c1d8628-20260707.{json,ndjson,csv}` 和同名截图/source，默认不提交。
...
- 当前剩余重点：P1 游玩质量和治理，包括 live 响应慢、narrator repair/judge 依赖、20 回合节奏、重复闭关/突破循环、叙事/权威状态落账，以及 `tests/web/test_web_api.py`、`web/backend/service.py`、`web/backend/database_postgres.py`、`src/agens_novel/engine/game_engine.py` 的小步复杂度治理。动态开局 fallback 可玩但不等于 live-model 成功，后续 Chrome 验收仍需记录 fallback 布尔。
