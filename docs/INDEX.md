...
- 系统模型为默认游玩能力；个人模型 Key 的目标产品边界是浏览器会话内使用、不写入数据库，当前实现差异见 `NEXT_GOVERNANCE_BACKLOG.md`。
...
- 2026-08-11 当前系统默认模型的 v2 严格开局、20 回合 smoke 与自然终局长局已在普通本机 Web 应用完成；长局第 87 回合飞升。证据和限制见 `PROJECT_AUDIT.md`，不扩展为其他模型、v3 或生产结论。
...
| `docs/PRD.md` | 玩家产品范围、首发 v2 与后续 v3 的验收边界 |
...
2. 产品与玩法：`docs/PRD.md` -> `docs/GAME_MODE_SPEC.md` -> `docs/USER_TUTORIAL.md`
