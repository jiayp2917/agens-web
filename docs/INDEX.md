...
- 最新本地代码基线：已包含脱敏模型性能观测、narrator prompt history soft-cap、2026-07-04 属性尺度清理、动态开局生成链路和 2026-07-06 治理审计执行（密钥 marker 统一、`WebGameService` 拆分、flow 耦合收敛、死代码清理）；`compileall` passed，`tests\web` 73 passed，全量 `pytest -q` 501 passed，前端 build passed，`git diff --check` 仅 LF/CRLF warning。
