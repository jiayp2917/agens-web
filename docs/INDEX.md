...
- 最新本地代码基线：已包含脱敏模型性能观测、P1 模型效率切片（普通回合 repair 已降为 0/20）、2026-07-04 属性尺度清理和动态开局生成链路；`compileall` passed，`tests\web` 73 passed，全量 `pytest -q` 516 passed，前端 build passed，`git diff --check` 仅 LF/CRLF warning。
