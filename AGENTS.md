...
### 移动端
- `mobile/main.py` — Kivy App 入口（平台引导 + 内置 Key 注入）
- `mobile/screens/` — 5 个 Screen（game/settings/save/combat/tutorial）
...
- **Engine 通过 callbacks 解耦 UI** — `on_narrative/on_error/on_combat_update` 等回调由 UI 层注册
...
445 个测试覆盖：命令解析、Agent 输出解析、游戏回合循环、存读档、状态 delta、渲染、破坏性输入、9 境界突破、战斗系统、真 LLM 端到端。
