...
> **维护说明**：本节是准则 source of truth。`CLAUDE.md` 保留逐字副本供 Claude Code 读取；改动必须同步两份（仓库内）。`D:\chat\CLAUDE.md` 是上级工作区治理文件，不在本仓库同步范围。
...
- **Narrator**：生成叙事和下一回合 A/B/C/D 选项建议（`state_update` 仅作传输诊断，不落库）；规则引擎会补齐固定 D 气运语义。
