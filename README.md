...
- 最新本地自动化基线：本地 PostgreSQL 可用；narrator prompt history soft-cap 修复后 `compileall` passed，`tests\web` 73 passed，全量 `pytest -q` 501 passed，前端 build passed，`git diff --check` 仅 LF/CRLF warning。
...
- 尚未完成：P1 游玩质量和治理，包括 live 响应慢、narrator 输出结构不稳定、重复闭关/突破循环、叙事/权威状态落账和大文件复杂度治理。
