...
- 最新本地自动化基线：本地 PostgreSQL 可用；`tests\web` 63 passed；带 `TEST_DATABASE_URL` 的全量 `pytest -q` 428 passed / 4 xfailed；前端 build 通过。
...
如果 `pg_isready` 已接受连接，直接跑测试；不要删除 `.tmp\pg-test-20260626-55432`。
